...
public class OidcClient extends BaseClient<OidcCredentials, OidcProfile> {
    ...
    protected void internalInit() {
        ...
        this.authParams.put("client_id", clientId);
        this.authParams.put("client_secret", secret);
        ...
        try {
            ...
            jwkSet = JWKSet.parse(resourceRetriever.retrieveResource(oidcProvider.getJWKSetURI().toURL()).getContent());
        ...
        clientAuthentication = getClientAuthentication(method);
        // Init JWT decoder
        this.jwtDecoder = new DefaultJWTDecoder();
        initJwtDecoder(jwtDecoder, jwkSet);
    ...
    protected RedirectAction retrieveRedirectAction(WebContext context) {
        ...
        String location = oidcProvider.getAuthorizationEndpointURI().toString() + "?" + queryString;
    ...
    protected OidcCredentials retrieveCredentials(WebContext context) throws RequiresHttpAction {
        ...
        try {
            response = AuthenticationResponseParser.parse(redirectURI, parameters);
        ...
        assert successResponse.getState().equals(context.getSessionAttribute(STATE_ATTRIBUTE));
    ...
    protected OidcProfile retrieveUserProfile(OidcCredentials credentials, WebContext context) {
        ...
        TokenRequest request = new TokenRequest(oidcProvider.getTokenEndpointURI(), clientAuthentication,
                new AuthorizationCodeGrant(credentials.getCode(), this.redirectURI, clientAuthentication.getClientID()));
        ...
        try {
            ...
            if (response instanceof TokenErrorResponse) {
                throw new TechnicalException("Bad token response " + response);
            }
            logger.debug("Token response successful");
            OIDCAccessTokenResponse tokenSuccessResponse = (OIDCAccessTokenResponse) response;

            // User Info request
            UserInfoRequest userInfoRequest = new UserInfoRequest(oidcProvider.getUserInfoEndpointURI(),
            ...
            if (userInfoResponse instanceof UserInfoErrorResponse) {
                throw new TechnicalException("Bad User Info response " + userInfoResponse);
            }
            UserInfoSuccessResponse userInfoSuccessResponse = (UserInfoSuccessResponse) userInfoResponse;

            // Check ID Token
            ReadOnlyJWTClaimsSet claimsSet = jwtDecoder.decodeJWT(tokenSuccessResponse.getIDToken());
            if (useNonce()) {
            ...
            OidcProfile profile = new OidcProfile(claimsSet, userInfoSuccessResponse.getUserInfo(),
    ...
    private ClientAuthenticationMethod getClientAuthenticationMethod() {
        return (oidcProvider.getTokenEndpointAuthMethods() != null && oidcProvider.getTokenEndpointAuthMethods().size() > 0) ? oidcProvider
    ...
    private boolean useNonce() {
        return Boolean.parseBoolean(authParams.get(USE_NONCE_PARAM));
    }
    ...
    private ClientAuthentication getClientAuthentication(ClientAuthenticationMethod method) {
        try {
            if (ClientAuthenticationMethod.CLIENT_SECRET_POST.equals(method)) {
                return ClientSecretPost.parse(authParams);
            }
        } catch (ParseException e) {
            throw new TechnicalException(e);
        }
